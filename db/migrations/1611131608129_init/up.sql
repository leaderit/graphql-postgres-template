...
END;
$$;
CREATE FUNCTION public.set_current_timestamp_uploaded() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
  _new record;
...
COMMENT ON COLUMN public.files.storage_id IS 'File storage id';
COMMENT ON COLUMN public.files.location_id IS 'File location in the storage';
